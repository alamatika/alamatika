...
const [selectedSeasonId, setSelectedSeasonId] =
  ...
  useEffect(() => {
  async function loadStoryAndSeason() {
    ...
    const [
      ...
      supabase
        .from("seasons")
        .select("id, story_id, season_number, title")
    ...
    if (seasonError) {
      console.error("Failed to load seasons:", seasonError);
    } else {
    ...
    if (seasonId) {
      const season = seasonData?.find(
        (item) =>
          String(item.id) === String(seasonId)
      );
  ...
  alert(
    "Please select a story and season."
  );
  return;
}
...
const { data: selectedSeason, error: seasonError } =
  await supabase
    .from("seasons")
    .select("id, story_id, season_number, title")
...
if (seasonError || !selectedSeason) {
  console.error("Failed to verify selected season:", seasonError);
  alert("The selected story/season could not be verified.");
  return;
}

const { data: selectedStory, error: storyError } =
  await supabase
    .from("stories")
    .select("id, title")
...
if (storyError || !selectedStory) {
  console.error("Failed to verify selected story:", storyError);
  alert("The selected story could not be verified.");
  return;
}
...
const { error } = await supabase
    ...
    alert("Chapter saved!");
      ...
      router.push(
  `/creator/stories/${selectedStoryId}/seasons/${selectedSeasonId}`
);
  }
    ...
    <CreatorGuard>
      <main className="min-h-screen bg-black text-white">
        ...
        <section className="max-w-3xl mx-auto pt-32 px-6">
          ...
          <div className="bg-zinc-900 rounded-2xl p-6 sm:p-8 mt-10 border border-yellow-500/30">
  ...
  <div className="space-y-4">
    ...
    <div>
      ...
      <select
        ...
        className="w-full bg-zinc-800 rounded-xl p-4 border border-yellow-500/30 outline-none focus:border-yellow-500 disabled:opacity-50"
      >
        <option value="">
          Select Season
        </option>
