...
console.log("PURCHASE DATA:", purchaseData);
  ...
  chaptersData?.map((c) => [c.id, c.title]) ?? []
);
...
purchaseData?.forEach((purchase) => {
  const id = purchase.chapter_id;

  const title =
    chapterMap.get(id) ?? `Chapter ${id}`;
...
<div className="bg-zinc-900 border border-yellow-500 rounded-2xl p-4 sm:p-8 mb-8 sm:mb-12">
  ...
  {bestSelling.length === 0 ? (
    ...
    <div className="space-y-3">
      ...
      {bestSelling.map((chapter, index) => (

        <div
          key={chapter.title}
          className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 border-b border-zinc-800 pb-3 hover:bg-zinc-800 rounded-lg px-3 py-3 transition"
        >
...
<div className="bg-zinc-900 border border-yellow-500 rounded-2xl p-8 mb-12">
  ...
  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
    ...
    <Link href="/admin/chapters" className="bg-zinc-800 rounded-xl p-5 text-center hover:bg-zinc-700 transition">
       Chapters
    </Link>

    <Link href="/admin/chapters/new" className="bg-zinc-800 rounded-xl p-5 text-center hover:bg-zinc-700 transition">
      ➕ New Chapter
    </Link>

    <Link href="/admin/characters" className="bg-zinc-800 rounded-xl p-5 text-center hover:bg-zinc-700 transition">
      👥 Characters
    </Link>

    <Link href="/admin/news" className="bg-zinc-800 rounded-xl p-5 text-center hover:bg-zinc-700 transition">
      📰 News
    </Link>

    <Link href="/admin/lore" className="bg-zinc-800 rounded-xl p-5 text-center hover:bg-zinc-700 transition">
      📜 Lore
    </Link>

    <Link href="/admin/assets" className="bg-zinc-800 rounded-xl p-5 text-center hover:bg-zinc-700 transition">
      🗂 Assets
    </Link>

    <Link href="/admin/settings" className="bg-zinc-800 rounded-xl p-5 text-center hover:bg-zinc-700 transition">
      ⚙ Settings
    </Link>
